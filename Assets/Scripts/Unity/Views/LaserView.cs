...
public class LaserView : MonoBehaviour, IAttackVisual
{
    ...
    {
        // Dọn dẹp kết quả cũ để trả về Pool (tránh leak RAM)
        ...
        // ÉP LineRenderer cập nhật số lượng điểm ngay lập tức
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < _targetViews.Count; i++)
        {
            // Kiểm tra quái còn sống và View vẫn tồn tại
            if (_targetViews[i] != null && _targetViews[i].Enemy != null && !_targetViews[i].Enemy.IsDead)
            {
    ...
    public void TurnOff() => GameController.Instance.PoolManager.ReturnPool(_prefab, gameObject);
